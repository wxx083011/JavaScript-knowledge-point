...
* 使用gulp插件
  ...
  * 处理css
    ...
    * 配置编码
      ...
      gulp.task('cssTask',['lessTask'], function () {
        ...
        return gulp.src('src/css/*.css')//指出准备操作的文件
            .pipe(concat('built.css'))//合成到那个文件
            .pipe(gulp.dest('dist/css'))//上面合成的文件要放在那个目录下
            .pipe(rename({suffix: '.min'}))//准备要被压缩的文件加个后缀名
            .pipe(cleanCSS({compatibility: 'ie8'}))//这个是css的压缩，参数可有可无，有参数代表说明兼容那个浏览器
            .pipe(gulp.dest('dist/css'));//压缩的css文件放在哪里，
      });
  ...
  * 自动编译(半自动)
    ...
    * 扩展(不要每次都引入包，直接只用引入一条，然后每一种方法都加上一个$)
        * 打包加载gulp插件
        * 前提：将插件下载好。
        ...
        * 使用方法：
            ...
            * 没有return的时候是同步，有return是异步，主要是异步的
            * less编译完 在编译css在 css中再传入一个参数，gulp("css",[* 任务化，基流化，、"less"],function(){}),加一个依赖就可以实现同步
            * src/js/*.js这个代表的是src下的js正下的所有js文件==src/js/**/*.js这个代表的是src下的js下的所有js文件，只要在下面的都是
            * .pipe(cleanCSS({compatibility:"ie8"}))参数可写可不写，写了是说明一下兼容浏览器版本，不写也行
            * 在用插件组合时  要记得原来的插件名字是什么，就var什么
